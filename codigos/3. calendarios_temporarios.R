...
pacman::p_load(gtfstools,dplyr,data.table,googlesheets4,Hmisc)

ano_gtfs <- '2023'
mes_gtfs <- '06'
quinzena_gtfs <- '02'

endereco_gtfs <- paste0("../../dados/gtfs/",ano_gtfs,"/sppo",
                             "_",ano_gtfs,"-",mes_gtfs,"-",quinzena_gtfs,"Q_PROC.zip")

gtfs <- read_gtfs(endereco_gtfs)

tabela_desvios <- read_sheet("1L7Oq1vqG5S_uOs_NdqgF4HG-Ac6gEyZrzQJYLpZH3OI",'linhas_desvios')
...
gtfs$trips <- gtfs$trips %>% 
  mutate(service_id = case_when(service_id=='U'&route_id %nin% rotas_afetadas_desvios ~ 'U_REG',
                                service_id=='S'&route_id %nin% rotas_afetadas_desvios ~ 'S_REG',
                                service_id=='D'&route_id %nin% rotas_afetadas_desvios ~ 'D_REG',
                                TRUE ~ service_id))

gtfs$calendar <- gtfs$calendar %>% 
  mutate(service_id = case_when(service_id=='U' ~ 'U_REG',
                                service_id=='S' ~ 'S_REG',
                                service_id=='D' ~ 'D_REG',
                                TRUE ~ service_id))
...
for(i in 1:length(lista_eventos)){
  ...
  gtfs$trips <- gtfs$trips %>% 
    mutate(service_id = case_when(service_id=='U'& route_id %in% linhas_afetadas_evento ~ paste0('U_DESAT_',evento),
  ...
  calendarios_novos <- gtfs$calendar %>% 
    filter(service_id %in% c(paste0('U_',evento),
                             paste0('S_',evento),
                             paste0('D_',evento))) %>% 
    mutate(service_id = case_when(service_id==paste0('U_',evento) ~ paste0('U_DESAT_',evento),
  ...
  excecoes_datas_at <- gtfs$calendar_dates %>% 
    ...
    mutate(service_id = case_when(service_id=='U' ~ paste0('U_',evento),
  ...
  excecoes_datas_desat <- gtfs$calendar_dates %>% 
    ...
    mutate(service_id = case_when(service_id=='U' ~ paste0('U_DESAT_',evento),
  ...
  calendar_dates_incluir <- tabela_evento %>%
    ...
    mutate(service_id = case_when(dia %in% c(2:6) ~ paste0('U_',evento),
  ...
  calendar_dates_excluir <- tabela_evento %>%
    ...
    mutate(dia = lubridate::wday(date)) %>% 
    mutate(service_id = case_when(dia %in% c(2:6) ~ paste0('U_DESAT_',evento),
  ...
  calendar_dates_evento <- rbindlist(list(calendar_dates_incluir,
...
feed_info <- gtfs$feed_info %>% 
  mutate(feed_start_date = lubridate::ymd(feed_start_date),
