...
endereco_sppo <- paste0("../../dados/gtfs/", ano_gtfs, "/sppo",
...
tarifas_frescao <- fread("../../dados/insumos/frescao/tarifas.csv") %>%
  ...
  mutate(agency_id = case_when(agency_id == "22002" ~ "IS",
                               agency_id == "22003" ~ "IN",
                               agency_id == "22004" ~ "TC",
                               agency_id == "22005" ~ "SC")) %>% 
...
endereco_brt <- paste0("../../dados/gtfs/",ano_gtfs,"/brt",
                        "_",ano_gtfs,"-",mes_gtfs,"-",quinzena_gtfs,"Q_PROC.zip")
...
#write_gtfs(gtfs_filt_brt,"D:/temp/gtfs_brt_rio.zip")
...
gtfs_combi$routes <- as.data.table(gtfs_combi$routes) %>% 
  ...
  mutate(route_color = case_when(agency_id == "22002" ~ "FCC417",
  ...
  mutate(route_text_color = case_when(agency_id == "22002" ~ "000000",
